...
def load_feat(d):
    ...
    conf_node_feats = None
    if os.path.exists('DATA/{}/{}_conf_node_feats_50neb.pt'.format(d, d)):
        conf_node_feats = torch.load('DATA/{}/{}_conf_node_feats_50neb.pt'.format(d, d))
        conf_node_feats = torch.tensor(np.array(conf_node_feats))
        if conf_node_feats.dtype == torch.bool:
            conf_node_feats = conf_node_feats.type(torch.float32)

    return node_feats, edge_feats, conf_node_feats
