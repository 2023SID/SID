...
def name_fn(args, mixer_configs):
    fn = 'results/%s_neighbors%d_edges%d_layers%d_%dhop_top'%(args.data, args.num_neighbors, args.max_edges, args.num_layers, args.sampled_num_hops)
...
def name_fn_ori(args, mixer_configs):
...
def get_args():
    ...
    parser.add_argument('--data', type=str, default='ESCORTS')
...
def load_all_data(args):
    ...
    # load feats
    node_feats, edge_feats, conf_node_feats = load_feat(args.data)
    ...
    conf_node_feat_dim = conf_node_feats.shape[1]
    ...
    args.conf_node_feat_dim = conf_node_feat_dim
    
    node_feats = node_feats.to(args.device) # here we only move node feats to cuda, not edges because too many edges
    
    return node_feats, edge_feats,conf_node_feats, g, df, args


def load_ori_model(args):
    # get model
    # get model
    ...
    edge_predictor_configs = {
        ...
        'dim_in_conf_node': args.conf_node_feat_dim
    }
    ...
    fn = name_fn_ori(args, mixer_configs)
    args.model_fn = fn + '_top1.0'+'.pt'
...
def load_model(args):
    ...
    top = str(args.top_percentage)
    args.model_fn = fn+top+'.pt'
    args.link_pred_result_fn = fn+top+'.json'
    print(args.model_fn)
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.model_fn) == False or args.regen_models:
        ...
        model = link_pred_train(model.to(args.device), args, g, df, node_feats, edge_feats,conf_node_feats)
