...
from construct_subgraph import construct_mini_batch_giant_graph, print_subgraph_data, construct_causal_mini_batch_giant_graph

from utils import row_norm, sym_norm


def run_for_ori(model, args, subgraphs, df, node_feats, edge_feats, mode):
    time_aggre = 0
    ...
    for ind in range(len(train_loader)):
        ...
        if args.use_graph_structure:
            ...
            subgraph_node_feats = compute_sign_feats(node_feats, df, cur_inds, num_of_df_links,
        ...
        pred_pos, pred_neg, ap, auc = model(inputs, has_temporal_neighbors, neg_samples, subgraph_node_feats)
        pred_pos, pred_neg = pred_pos.sigmoid(), pred_neg.sigmoid()
        time_aggre += (time.time() - start_time)

        all_pred_pos.append(pred_pos)
        all_pred_neg.append(pred_neg)
        all_ap.append(ap)
        all_auc.append(auc)
    ...
    print('%s mode with time %.4f, average precision %.4f, auc score %.4f' % (mode, time_aggre, ap, auc))
    return all_pred_pos,all_pred_neg,ap,auc



def run_for_fid(model, args, subgraphs, df, node_feats, edge_feats, confounder, mode):
    time_aggre = 0

    ###################################################
    # compute + training + fetch all score
    ...
    for ind in range(len(train_loader)):
        ...
        subgraph_data_for_node = construct_mini_batch_giant_graph(subgraph_data, args.max_edges)
        if args.use_graph_structure:
            num_subgraphs = len(mini_batch_inds)
            num_of_df_links = len(subgraph_data_list) // (cached_neg_samples + 2)
            subgraph_node_feats = compute_sign_feats(node_feats, df, cur_inds, num_of_df_links,
                                                     subgraph_data_for_node['root_nodes'], args)
        ...
        pred_pos, pred_neg, ap, auc = model(subgraph_data, neg_samples, edge_feats, args, subgraph_node_feats, confounder)
        pred_pos, pred_neg = pred_pos.sigmoid(), pred_neg.sigmoid()
    ...
    print('%s mode with time %.4f, average precision %.4f, auc score %.4f' % (mode, time_aggre, ap, auc))
    return  all_pred_pos,all_pred_neg,all_ap,all_auc

def run(model, optimizer, args, subgraphs, df, node_feats, edge_feats, confounder,mode):
    time_aggre = 0
    ...
    for ind in range(len(train_loader)):
        ...
        start_time = time.time()
        loss, ap, auc, ap_inv1, auc_inv1, ap_inv2, auc_inv2 = model(subgraph_data, neg_samples,edge_feats,args,subgraph_node_feats,confounder)
        ...
        all_ap.append(ap)
        all_auc.append(auc)
        all_ap_inv1.append(ap_inv1)
        all_auc_inv1.append(auc_inv1)
        all_ap_inv2.append(ap_inv2)
        all_auc_inv2.append(auc_inv2)
    ...
    ap, auc = sum(all_ap)/len(all_ap), sum(all_auc)/len(all_auc)
    ap_inv1, auc_inv1 = sum(all_ap_inv1) / len(all_ap_inv1), sum(all_auc_inv1) / len(all_auc_inv1)
    ap_inv2, auc_inv2 = sum(all_ap_inv2) / len(all_ap_inv2), sum(all_auc_inv2) / len(all_auc_inv2)
    print('%s mode with time %.4f, ap %.4f, auc %.4f, ap_inv1 %.4f, auc_inv1 %.4f, ap_inv2 %.4f, auc_inv2 %.4f, loss %.4f'%(mode, time_aggre, ap, auc,ap_inv1,auc_inv1,ap_inv2,auc_inv2, loss.item()))
    if mode=='test':
        return ap, auc, ap_inv1, auc_inv1, ap_inv2, auc_inv2, loss.item()
    return ap, auc, loss.item()

def link_pred_train(model, args, g, df, node_feats, edge_feats,conf_node_feats):
    ...
    for epoch in range(args.epochs):
        ...
        train_ap, train_auc, train_loss = run(model, optimizer, args, train_subgraphs, df,
                                              node_feats, edge_feats,conf_node_feats, mode='train')
        ...
        with torch.no_grad():
            ...
            valid_ap, valid_auc, valid_loss = run(copy.deepcopy(model), None, args, valid_subgraphs, df,
                                                 node_feats, edge_feats,conf_node_feats, mode='valid')
            # second variable (optimizer) is only required for training
            start_time = time.time()
            test_ap,  test_auc,test_ap_inv1,  test_auc_inv1,test_ap_inv2,  test_auc_inv2,  test_loss  = run(copy.deepcopy(model), None, args, test_subgraphs,  df,
                                                  node_feats, edge_feats,conf_node_feats, mode='test')
            end_time = time.time()
        ...
        if valid_ap > best_ap:
            ...
            best_test_ap_inv1, best_test_auc_inv1 = test_ap_inv1, test_auc_inv1
            best_test_ap_inv2, best_test_auc_inv2 = test_ap_inv2, test_auc_inv2
    ...
    print('ap %.4f, auc %.4f,ap_inv1 %.4f, auc_inv1 %.4f,ap_inv2 %.4f, auc_inv2 %.4f'%(best_test_ap, best_test_auc,best_test_ap_inv1, best_test_auc_inv1,best_test_ap_inv2, best_test_auc_inv2))
